...
public class Grafo<TIPO> {
    ...
    private ArrayList<Vertice<TIPO>> verticesProfessores;
    private ArrayList<Vertice<TIPO>> verticesDisciplinas;
    ...
    public Grafo(String tipoGrafo) {
        ...
        this.verticesProfessores = new ArrayList<Vertice<TIPO>>();
        this.verticesDisciplinas = new ArrayList<Vertice<TIPO>>();
    }
    ...
    //Adicionar aresta
    public void adicionarAresta(int peso, TIPO codVerticeInicio, TIPO codVerticeFim, String cod) {
        ...
        Aresta<TIPO> aresta = new Aresta<TIPO>(peso, inicio, fim, cod);
    ...
    /**
     * Métodos exclusivos do trabalho final
     * 
     */

    //Aloca professores
    public void adicionarVerticeProfessores(TIPO codVertice) {
        Vertice<TIPO> novoVertice = new Vertice<TIPO>(codVertice);
        this.vertices.add(novoVertice);
        this.verticesProfessores.add(novoVertice);
        this.qntVertices++;
    }

    //Aloca disciplinas
    public void adicionarVerticeDisciplinas(TIPO codVertice) {
        Vertice<TIPO> novoVertice = new Vertice<TIPO>(codVertice);
        this.vertices.add(novoVertice);
        this.verticesDisciplinas.add(novoVertice);
        this.qntVertices++;
    }

    //Tenta alocar um horário para a professor
    public void alocaHorario(TIPO professor, TIPO disciplina){

        // Handler de resposta
        String response[] = new String[2];
        response[0] = "error";
        response[1] = "Não foi possível alocar um horário";

        /*
		 * Criando arraylist de horários
		 * 
		 * Premissas: 
		 * 
		 * - Para a criação dos horários levamos em consideração que cada disciplina terá a carga horária de 80h, ou seja, 2 aulas por semana.
		 * - Também estamos levando em consideração que as duas aulas serão dadas no mesmo dia, seguindo o atual formato de horários da PUC São Gabriel.
		 * - Estamos também considerando apenas aulas no turno da noite.	
		 *   
		 */
		ArrayList<String> horarios = new ArrayList<String>();
		horarios.add("Segunda-feira");
		horarios.add("Terca-feira");
		horarios.add("Quarta-feira");
		horarios.add("Quinta-feira");
		horarios.add("Sexta-feira");

        //Coleta periodo
        String[] disciplinaArray = new String[2];
        String disciplinaString = disciplina.toString();
        disciplinaArray = disciplinaString.split(" ");
        String periodo = disciplinaArray[1];

        //Verifica se possui horário livre
        for(String horario : horarios){
            if(this.horarioLivre(horario, periodo) && this.professorHorarioLivre(horario, professor)){
                this.adicionarAresta(0, professor, disciplina, horario);
                response[0] = "success";
                response[1] = "Horário alocado! " + professor + " / " + disciplina + " / " + horario;
                break;
            }
        }

        System.out.println(response[1]); //Debug
        
    }

    //Analise se o professor tem horário livre
    public boolean professorHorarioLivre(String horario, TIPO professor){
        boolean isHorarioLivre = true;
        Vertice<TIPO> vertice = this.getVertice(professor);
        for(Aresta<TIPO> aresta : vertice.getArestasSaida()){
            if(aresta.getCod().equals(horario)){
                isHorarioLivre = false;
                //System.out.println("Horario ocupado!"); //Debug
            }
        }
        return isHorarioLivre;
    }

    //Analisa se tem horário livre no periodo indicado (entre professores)
    public boolean horarioLivre(String horario, String periodo){

        boolean isHorarioLivre = true;
        ArrayList<Vertice<TIPO>> disciplinasDoMesmoPeriodo = new ArrayList<Vertice<TIPO>>();
        disciplinasDoMesmoPeriodo = this.disciplinasDoMesmoPeriodo(periodo);

        for(Vertice<TIPO> disciplinaDoMesmoPeriodo : disciplinasDoMesmoPeriodo){
            for(Aresta<TIPO> arestaDisciplinaDoMesmoPeriodo : disciplinaDoMesmoPeriodo.getArestasSaida()){
                if(arestaDisciplinaDoMesmoPeriodo.getCod().equals(horario)){
                    isHorarioLivre = false;
                    //System.out.println("Horario ocupado!"); //Debug
                }
            }
    ...
    //Retorna o nome de todas as disciplinas do mesmo período
    public ArrayList<Vertice<TIPO>> disciplinasDoMesmoPeriodo(String periodo){

        ArrayList<Vertice<TIPO>> disciplinasDoMesmoPeriodo = new ArrayList<Vertice<TIPO>>();

        for(Vertice<TIPO> disciplina : this.verticesDisciplinas){
            TIPO nomeDisciplinaVertice = disciplina.getCodVertice(); 
            String nomeDisciplinaString = nomeDisciplinaVertice.toString();
            String[] disciplinaArray = new String[2];
            disciplinaArray = nomeDisciplinaString.split(" ");
            if(periodo.equals(disciplinaArray[1])){
                disciplinasDoMesmoPeriodo.add(disciplina);
            }
        }

        return disciplinasDoMesmoPeriodo;

    }

    //Retorna tabela de horários do professor
    public void tabelaDeHorariosProfessor(TIPO professor){
        try{
            Vertice<TIPO> vertice = this.getVertice(professor);
            System.out.println("PROFESSOR: " + vertice.getCodVertice());
            for(Vertice<TIPO> verticeVizinho : vertice.getVizinhos()){
                System.out.print("Disciplina: " + verticeVizinho.getCodVertice() + " / ");
                for(Aresta<TIPO> aresta : verticeVizinho.getArestasSaida()){
                    System.out.print(aresta.getCod());
                }
                System.out.println("");
            }
    ...
    //Retorna os horários por período
    public void tabelaDeHorariosPeriodo(String periodo){
        ...
        for(Vertice<TIPO> disciplina : disciplinasDoMesmoPeriodo){
            disciplinaString = disciplina.getCodVertice().toString();
            disciplinaArray = disciplinaString.split(" ");
            System.out.print(disciplinaArray[0] + " / ");
            for(Aresta<TIPO> aresta : disciplina.getArestasSaida()){
                System.out.print(aresta.getCod() + " / ");
                System.out.print(aresta.getInicio().getCodVertice());
            }
            System.out.println("");
        }
